...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $success_msg[] = 'Pengguna dihapus!';
   }else{
      $warning_msg[] = 'Pengguna sudah terhapus!';
   }
...
<section class="grid">
   ...
   <form action="" method="POST" class="search-form">
      <input type="text" name="search_box" placeholder="Cari pengguna..." maxlength="100" required>
   ...
   <div class="box">
      ...
      <p>Nomor : <a href="tel:<?= $fetch_users['number']; ?>"><?= $fetch_users['number']; ?></a></p>
      <p>Email : <a href="mailto:<?= $fetch_users['email']; ?>"><?= $fetch_users['email']; ?></a></p>
      <p>Listingan properti : <span><?= $total_properties; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="delete_id" value="<?= $fetch_users['id']; ?>">
         <input type="submit" value="Hapus pengguna" onclick="return confirm('hapus pengguna ini?');" name="delete" class="delete-btn">
      </form>
   </div>
   <?php
      }
      ...
      echo '<p class="empty">belum ada pengguna yang ditambahkan!</p>';
   }
   ?>
