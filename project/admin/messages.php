...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $warning_msg[] = 'Pesan Sudah Terhapus!';
   }
...
<section class="grid">
   ...
   <form action="" method="POST" class="search-form">
      <input type="text" name="search_box" placeholder="Cari pesan..." maxlength="100" required>
   ...
   <div class="box">
      ...
      <p>Nomor : <a href="tel:<?= $fetch_messages['number']; ?>"><?= $fetch_messages['number']; ?></a></p>
      <p>Pesan : <span><?= $fetch_messages['message']; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="delete_id" value="<?= $fetch_messages['id']; ?>">
         <input type="submit" value="Hapus pesan" onclick="return confirm('Hapus Pesan Ini?');" name="delete" class="delete-btn">
      </form>
   </div>
   <?php
      }
   }elseif(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
      echo '<p class="empty">Pencarian tidak ditemukan</p>';
   }else{
