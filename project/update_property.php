...
if(isset($_POST['update'])){
   ...
   $update_listing = $conn->prepare("UPDATE `property` SET property_name = ?, address = ?, price = ?, type = ?, offer = ?, furnished = ?, bedroom = ?, bathroom = ?, carpet = ?, total_floors = ?, loan = ?, lift = ?, security_guard = ?, play_ground = ?, garden = ?, water_supply = ?, power_backup = ?, parking_area = ?, gym = ?, shopping_mall = ?, hospital = ?, school = ?, market_area = ?, description = ? WHERE id = ?");   
   $update_listing->execute([$property_name, $address, $price, $type, $offer, $furnished, $bedroom, $bathroom, $carpet, $total_floors, $loan, $lift, $security_guard, $play_ground, $garden, $water_supply, $power_backup, $parking_area, $gym, $shopping_mall, $hospital, $school, $market_area, $description, $update_id]);

   $success_msg[] = 'Listing Terupdate!';
...
<section class="property-form">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="box">
         ...
         <input type="text" name="property_name" required maxlength="50" placeholder="Buatlah judul yang menarik" value="<?= $fetch_property['property_name']; ?>" class="input">
      </div>
      <div class="flex">
         <div class="box">
            <p>Harga <span>*</span></p>
            <input type="number" name="price" required min="0" max="9999999999" maxlength="10" value="<?= $fetch_property['price']; ?>" placeholder="Pastikan harganya benar" class="input">
         </div>
         <!-- <div class="box">
         ...
         <div class="box">
            ...
            <input type="text" name="address" required maxlength="100" placeholder="Kecamatan, Provinsi" class="input" value="<?= $fetch_property['address']; ?>">
         </div>
         <div class="box">
         ...
         <div class="box">
            ...
            <input type="number" name="total_floors" required min="0" max="99" maxlength="2" placeholder="Total Lantai" class="input" value="<?= $fetch_property['total_floors']; ?>">
         </div>
         <div class="box">
      ...
      <div class="box">
         ...
         <textarea name="description" maxlength="1000" class="input" required cols="30" rows="10" placeholder="Buatlah deskripsi yang menarik untuk properti anda" ><?= $fetch_property['description']; ?></textarea>
      </div>
      <div class="checkbox">
         <div class="box">
            ...
            <p><input type="checkbox" name="garden" value="yes" <?php if($fetch_property['garden'] == 'yes'){echo 'checked'; } ?> />Taman</p>
            <p><input type="checkbox" name="water_supply" value="yes" <?php if($fetch_property['water_supply'] == 'yes'){echo 'checked'; } ?> />Air</p>
            <p><input type="checkbox" name="power_backup" value="yes" <?php if($fetch_property['power_backup'] == 'yes'){echo 'checked'; } ?> />Daya Cadangan</p>
         </div>
         <div class="box">
            <p><input type="checkbox" name="parking_area" value="yes" <?php if($fetch_property['parking_area'] == 'yes'){echo 'checked'; } ?> />Area Parkir</p>
            ...
            <p><input type="checkbox" name="hospital" value="yes" <?php if($fetch_property['hospital'] == 'yes'){echo 'checked'; } ?> />Rumah Sakit</p>
            <p><input type="checkbox" name="school" value="yes" <?php if($fetch_property['school'] == 'yes'){echo 'checked'; } ?> />Sekolah</p>
            <p><input type="checkbox" name="market_area" value="yes" <?php if($fetch_property['market_area'] == 'yes'){echo 'checked'; } ?> />Supermarket</p>
         </div>
      </div>
      <div class="box">
      ...
      <div class="flex"> 
         <div class="box">
            ...
            <input type="submit" value="Hapus gambar" name="delete_image_02" class="inline-btn" onclick="return confirm('Hapus gambar ini?');">
            <?php } ?>
            <p>Kamar Mandi</p>
         ...
         <div class="box">
            ...
            <input type="submit" value="Hapus gambar" name="delete_image_03" class="inline-btn" onclick="return confirm('Hapus gambar ini?');">
            <?php } ?>
            <p>Kamar Tidur</p>
         ...
         <div class="box">
            ...
            <input type="submit" value="Hapus gambar" name="delete_image_04" class="inline-btn" onclick="return confirm('Hapus gambar ini?');">
            <?php } ?>
            <p>Gambar Tambahan</p>
         ...
         <div class="box">
            ...
            <input type="submit" value="Hapus gambar" name="delete_image_05" class="inline-btn" onclick="return confirm('Hapus gambar ini?');">
            <?php } ?>
            <p>Gambar Tambahan</p>
            <input type="file" name="image_05" class="input" accept="image/*">
         </div>   
      </div>
      <input type="submit" value="Update Properti" class="btn" name="update">
   </form>
   <?php
      }
   }else{
      echo '<p class="empty">Properti Tidak Ditemukan!<a href="post_property.php" style="margin-top:1.5rem;" class="btn">Tambah Baru</a></p>';
   }
   ?>
